...
import net.osmand.plus.routing.BluetoothModule;
...
//MITUL
	...
	private Term empty;
        ...
        private void bt_send_to_doggi(TurnType t)
        { 
           String direction="";
	    ...
	    logPrinter.println("VoiceRouter:updateStatus: playMakeTurn null");
            try{
                bt_handle.write(direction.getBytes());
            }
            catch (IOException e) {logPrinter.println("BluetoothModule: write exception");}
        }
	...
	public VoiceRouter(RoutingHelper router, final OsmandSettings settings) {
		...
		logPrinter = new LogPrinter(4, "OsmAnd");
		empty = new Struct("");
	...
	protected void updateStatus(Location currentLocation, boolean repeat) {
		...
		float speed = DEFAULT_SPEED;
		//logPrinter.println("VoiceRouter:updateStatus");
		...
		if (!repeat) {
			if (dist == 0) {
				...
				playGoAhead(dist, getSpeakableStreetName(currentSegment, next));
				 logPrinter.println("VoiceRouter:updateStatus: needsInforming");
				return;
		...
		if ((repeat || statusNotPassed(STATUS_TURN)) && isDistanceLess(speed, dist, TURN_DISTANCE, TURN_DEFAULT_SPEED)) {
			if (next.distance < TURN_IN_DISTANCE_END && nextNextInfo != null) {
				logPrinter.println("VoiceRouter:updateStatus: playMakeTurn ");
                                bt_send_to_doggi(next.getTurnType());
